...
const Editform = ({id,fullname,phone,restaurantName,reservationDate,reservationTime,numberOfPeople,email,value,onClose}) => {
  ...
  const formattedMinimumTime = minimumTime.toTimeString().split(' ')[0]; 
      ...
      const handleSubmit =async (e) => {
        ...
        try {
          const response = await axios.put('http://localhost:8080/api/reservation/update', formData);
  ...
  return (
    ...
    <h1 className='h1-form'>{restaurantName}</h1>
    <form onSubmit={handleSubmit}>
      ...
      <div className="form-group" style={{marginLeft:'40px'}}>
        ...
        <input type="email" id="email" name="email" disabled className='input-form'  value={formData.email} onChange={handleChange} />
      </div>
      </div>
      <div style={{display:'flex'}}>
      <div className="form-group">
      ...
      <div className="form-group">
        ...
        <input type="date" id="reservationDate" name="reservationDate"   className='input-form' value={formData.reservationDate} onChange={handleChange} min={currentDate}/>
      </div>
      <div className="form-group" style={{marginLeft:'20px'}}>
        <label htmlFor="reservationTime" className='label-form'>Reservation Time</label>
        <input type="time" id="reservationTime" name="reservationTime"  className='input-form'  value={formData.reservationTime} onChange={handleChange} min="00:00"/>
      </div>
      </div>
